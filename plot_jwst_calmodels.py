...
def initialize_parser():
    ...
    parser.add_argument('-w', '--waverange',
                        choices=['all', 'nir', 'mir'],
                        default='all',
...
#    exit()
    ...
    cax = ax[0]
    cax.plot(astar['WAVELENGTH'], (astar['WAVELENGTH']**4)*astar['FLUX'],
             'k-', label="R = 300,000", alpha=0.25)
    cax.plot(astar_wave, (astar_wave**4)*astar_flux, 'b-',
             label="R ~ 3,000")
    cax.set_xscale('log')
    ...
    cax.set_ylabel("$\lambda^4 F(\lambda)$")
    cax.legend()

    gstar['WAVELENGTH'] *= 1e-4
    cax = ax[1]
    cax.plot(gstar['WAVELENGTH'], (gstar['WAVELENGTH']**4)*gstar['FLUX'],
             'k-', label="R = 300,000", alpha=0.25)
    cax.plot(gstar_wave, (gstar_wave**4)*gstar_flux, 'b-',
             label="R ~ 3,000")
    cax.set_yscale(ptype)
    ...
    cax = ax[2]
    cax.plot(wdstar['WAVELENGTH'], (wdstar['WAVELENGTH']**4)*wdstar['FLUX'],
             'k-', label="R = 30,000", alpha=0.25)
